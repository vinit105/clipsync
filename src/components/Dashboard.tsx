...
const Dashboard = () => {
  ...
  useEffect(() => {
    const handlePasteShortcut = async (e: ClipboardEvent) => {
      if (e.clipboardData) {
        const pastedText = e.clipboardData.getData("text");
        if (pastedText.trim()) {
          await handlePaste(pastedText);
        }
      }
    };
    ...
    return () => {
      document.removeEventListener("paste", handlePasteShortcut);
    };
  }, [activeTab, codeSnippets, links, texts]);
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      setCodeSnippets(
        codeSnap.docs.map((doc) => ({
          id: doc.id,
          content: doc.data().content,
        }))
      );
      setLinks(
        linkSnap.docs.map((doc) => ({
          id: doc.id,
          content: doc.data().content,
        }))
      );
      setTexts(
        textSnap.docs.map((doc) => ({
          id: doc.id,
          content: doc.data().content,
        }))
      );
  ...
  const renderModal = () =>
    ...
    createPortal(
      ...
      >
        <div
          className="bg-white w-full max-w-md h-[90vh] rounded-3xl shadow-xl p-6 relative flex flex-col"
        ...
        >
          ...
          <div className="flex-1 overflow-auto text-sm whitespace-pre-wrap text-gray-800 bg-gray-50 p-4 rounded-lg border border-gray-200">
  ...
  const Card = ({
    ...
    <div
      onClick={() => setModalContent(rawText)}
      className={`relative p-2 rounded-lg border shadow-sm text-sm cursor-pointer hover:shadow-md transition overflow-hidden h-[120px] ${
  ...
  const renderGrid = (type: "code" | "link" | "text") => {
    const items =
      type === "code" ? codeSnippets : type === "link" ? links : texts;

    return (
      <div
        className="grid grid-flow-col grid-rows-4 gap-2 max-h-[80vh] overflow-x-auto pr-1 p-1"
        style={{ gridAutoColumns: 250 }}
      >
        {items.map((item, i) => (
          <Card
            key={item.id}
            ...
            content={
              type === "code" ? (
                <pre className="bg-black text-white text-xs p-2 rounded whitespace-pre-wrap overflow-hidden">
  ...
  return (
    <div className="h-[calc(100vh-0px)] w-full flex flex-col overflow-hidden bg-green-100">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center px-4 py-2 sticky top-0 bg-green-100 z-20 w-full space-y-2 sm:space-y-0">
  {/* Top left: Title */}
  <h1 className="text-2xl font-bold whitespace-nowrap">📚 ClipSync Dashboard</h1>

  {/* Right section: Shortcut + Paste button */}
  <div className="flex flex-col sm:flex-row items-center sm:space-x-4 space-y-2 sm:space-y-0 sm:justify-end w-full sm:w-auto">
    <span className="text-sm text-gray-500 font-medium text-center sm:text-right whitespace-nowrap">
